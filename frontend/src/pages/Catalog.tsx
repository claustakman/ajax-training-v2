        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
          {tags.map(tag => (
            <button key={tag} onClick={() => toggleTag(tag)}
              style={{ padding: '8px 16px', borderRadius: 20, fontSize: 13, fontWeight: 500, background: selectedTags.includes(tag) ? 'var(--accent)' : 'var(--bg-card)', color: selectedTags.includes(tag) ? '#fff' : 'var(--text2)', border: `1px solid ${selectedTags.includes(tag) ? 'var(--accent)' : 'var(--border2)'}` }}>
              {tag}
            </button>
          ))}
        </div>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
              ...
              <button onClick={handleAiDescription} disabled={aiLoading}
                style={{ padding: '8px 14px', borderRadius: 6, fontSize: 12, fontWeight: 500, background: aiLoading ? 'var(--bg-input)' : 'var(--accent-light)', color: aiLoading ? 'var(--text3)' : 'var(--accent)', border: `1px solid ${aiLoading ? 'var(--border)' : 'var(--accent)'}` }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
              {AGE_GROUPS.map(ag => (
                <button key={ag} onClick={() => toggleArrItem('age_groups', ag)}
                  style={{ padding: '8px 14px', borderRadius: 20, fontSize: 12, fontWeight: 500, background: form.age_groups.includes(ag) ? 'var(--accent)' : 'var(--bg-input)', color: form.age_groups.includes(ag) ? '#fff' : 'var(--text2)', border: `1px solid ${form.age_groups.includes(ag) ? 'var(--accent)' : 'var(--border2)'}` }}>
                  {ag}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', gap: 8, paddingTop: 8 }}>
            {!isNew && (
              <button onClick={handleDelete} disabled={deleting}
                style={{ padding: '12px 24px', borderRadius: 8, fontSize: 14, fontWeight: 500, background: 'rgba(220,38,38,0.08)', color: 'var(--red)', border: '1px solid rgba(220,38,38,0.2)' }}>
